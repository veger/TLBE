...
function TestNewTracker.TestUniqueName()
    local baseTracker = Tracker.newTracker(game.players[1], "base")
    lu.assertEquals(baseTracker.name, "base", "with empty list use tracker type as name")

    local secondBaseTracker = Tracker.newTracker(game.players[1], "base", { baseTracker })
    lu.assertEquals(secondBaseTracker.name, "base-2", "with tracker 'base' already in the list, add '-2' to the name")

    local thirdBaseTracker = Tracker.newTracker(game.players[1], "base", { baseTracker, secondBaseTracker })
    lu.assertEquals(
    ...
    local playerTracker = Tracker.newTracker(game.players[1], "player", { baseTracker })
